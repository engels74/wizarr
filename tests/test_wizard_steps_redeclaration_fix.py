...
def test_wizard_steps_script_guard_prevents_redeclaration(page: Page, live_server):
    ...
    assert len(redeclaration_errors) == 0, (
        f"Found redeclaration errors: {redeclaration_errors}"
    )


def test_wizard_drag_drop_functionality_works_after_multiple_loads(
    ...
    assert wizard_manager_exists, (
        "WizardDragDropManager should be available after multiple script loads"
    )
    ...
    if page.query_selector(".wizard-steps"):
        assert sortable_containers > 0, (
            "Sortable containers should be properly attached"
        )
...
def test_no_lexical_declaration_errors_on_script_load(page: Page, live_server):
    ...
    lexical_errors = [
        error
        for error in console_errors
        if "can't access lexical declaration" in error.lower()
        and "before initialization" in error.lower()
    ]

    assert len(lexical_errors) == 0, (
        f"Found lexical declaration errors: {lexical_errors}"
    )
    ...
    reference_errors = [
        error
        for error in console_errors
        if "referenceerror" in error.lower()
        and any(
            func in error.lower()
            for func in [
    ...
    assert len(reference_errors) == 0, (
        f"Found reference errors for functions: {reference_errors}"
    )
...
def test_backward_compatibility_functions_still_work(page: Page, live_server):
    ...
    for func in compatibility_functions:
        assert func["exists"], (
            f"Backward compatibility function '{func['name']}' should be available"
        )
...
def test_wizard_steps_js_syntax_is_valid():
    ...
    try:
        ...
        result = subprocess.run(
            ["node", "-c", script_path], capture_output=True, text=True, timeout=10
        )

        if result.returncode != 0:
            # If node check fails, the syntax is invalid
            raise AssertionError(
                f"JavaScript syntax error in wizard-steps.js: {result.stderr}"
            )
...
def test_script_guard_implementation_details():
    ...
    assert (
        "console.log" in script_content
        or "return"
        in script_content.split("if (typeof window.WizardDragDropManager")[1].split(
            "} else {"
        )[0]
    )
