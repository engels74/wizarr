...
def test_wizard_step_category_migration_downgrade(migration_app, temp_db):
    ...
    with migration_app.app_context():
        ...
        with engine.connect() as conn:
            ...
            table_sql = result.fetchone()[0]
            assert "uq_step_server_pos" in table_sql, "Old unique constraint not restored"
    ...
    with migration_app.app_context():
        ...
        with engine.connect() as conn:
            ...
            try:
                ...
                conn.commit()
                assert False, "Should not allow duplicate (server_type, category, position)"
            except Exception as e:
                # Expected: unique constraint violation
                assert "UNIQUE constraint failed" in str(e) or "unique" in str(
                    e
                ).lower(), f"Expected unique constraint error, got: {e}"
                conn.rollback()
