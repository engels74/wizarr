...
def test_create_step_with_pre_invite_category(authenticated_client, session, plex_server):
...
def test_create_step_with_post_invite_category(authenticated_client, session, plex_server):
...
def test_edit_step_category_from_post_to_pre(authenticated_client, session, plex_server):
    ...
    session.expire_all()
    updated_step = WizardStep.query.get(step_id)
    assert updated_step is not None
    assert updated_step.category == "pre_invite"
    assert updated_step.title == "Changed to Pre Step"


def test_edit_step_category_from_pre_to_post(authenticated_client, session, plex_server):
...
def test_create_preset_with_pre_invite_category(authenticated_client, session, plex_server):
    """Test creating a preset step with pre_invite category."""
    # Note: This test assumes presets are available. If not, it will fail gracefully.
    response = authenticated_client.post(
    ...
    # If preset doesn't exist, we'll get an error flash, but the form should still work
    # We're testing that the category field is properly handled
    assert response.status_code in [302, 200]


def test_create_preset_with_post_invite_category(authenticated_client, session, plex_server):
...
def test_position_calculation_respects_category(authenticated_client, session, plex_server):
    ...
    existing_post_count = WizardStep.query.filter_by(
    ...
    pre_steps = WizardStep.query.filter_by(
        server_type="plex", category="pre_invite"
    ).order_by(WizardStep.position).all()
...
def test_create_simple_step_with_pre_invite_category(authenticated_client, session):
    ...
    step = WizardStep.query.filter_by(server_type="custom", category="pre_invite").first()
    assert step is not None
...
def test_create_simple_step_with_post_invite_category(authenticated_client, session):
    ...
    step = WizardStep.query.filter_by(server_type="custom", category="post_invite").first()
    assert step is not None
...
def test_create_step_invalid_category_rejected(authenticated_client, session, plex_server):
...
def test_category_persists_after_multiple_edits(authenticated_client, session, plex_server):
    ...
    session.expire_all()
    step = WizardStep.query.get(step_id)
    assert step.category == "pre_invite"
