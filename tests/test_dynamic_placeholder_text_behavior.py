...
class TestDynamicPlaceholderCreation:
    ...
    def test_pre_phase_shows_placeholder_when_becomes_empty_via_drag_drop(
        ...
        # Initially, neither phase should show visible placeholder since both have steps
        # The text appears in JavaScript but not in visible placeholder elements
        import re

        # Extract only content from visible placeholder divs (not JavaScript)
        placeholder_pattern = r'<div class="empty-phase-placeholder[^>]*>.*?</div>'
        placeholder_matches = re.findall(
            placeholder_pattern, initial_response_text, re.DOTALL
        )
        placeholder_content = " ".join(placeholder_matches)
        assert "No pre-invite steps" not in placeholder_content
        assert "No post-invite steps" not in placeholder_content
        ...
        # Post-phase should still have steps (no visible placeholder)
        # Check only visible placeholder elements, not JavaScript code
        import re

        placeholder_pattern = r'<div class="empty-phase-placeholder[^>]*>.*?</div>'
        placeholder_matches = re.findall(placeholder_pattern, response_text, re.DOTALL)
        placeholder_content = " ".join(placeholder_matches)
        assert "No post-invite steps" not in placeholder_content
    ...
    def test_post_phase_shows_placeholder_when_becomes_empty_via_drag_drop(
        ...
        # Pre-phase should still have steps (no visible placeholder)
        # Check only visible placeholder elements, not JavaScript code
        import re
...
class TestPlaceholderRemovalBehavior:
    ...
    def test_placeholder_removed_when_empty_pre_phase_gets_step(
        ...
        # Pre-phase should no longer show visible placeholder
        # Check only visible placeholder elements, not JavaScript code
        import re
    ...
    def test_placeholder_removed_when_empty_post_phase_gets_step(
        ...
        # Post-phase should no longer show visible placeholder
        # Check only visible placeholder elements, not JavaScript code
        import re
...
class TestJavaScriptIntegration:
    ...
    def test_updateemptystate_function_exists_in_response(
        ...
        # Verify inline JavaScript function is included
